...
class GoPlayer:
    """An abstract class representing different algorithms to play Go
...
class FullyRandom(GoPlayer):
    """A Go player that plays randomly by choosing its next move completely randomly
    with no respect for the state of current board, except for checking if the move is valid
    """
...
class UserGoPlayer(GoPlayer):
    ...
    Note that the game tree attribute is currently unused, but may eventually be available to the user for display
    """
